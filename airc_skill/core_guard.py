...
_TECH_TERM_PATTERNS = (
    ...
    re.compile(r"[A-Za-z][A-Za-z0-9_-]{2,}(?=[\u4e00-\u9fff，。；、:：\s]|$)"),
)
...
def snapshot_core_content(text: str, suffix: str) -> CoreSnapshot:
    return CoreSnapshot(
        ...
        numbers=normalize_numeric_tokens_for_integrity(text),
        ...
        captions=extract_caption_spans(text),
        caption_has_colon_period=any("：。" in caption for caption in extract_caption_spans(text)),
...
def compare_core_snapshots(original: CoreSnapshot, revised: CoreSnapshot) -> dict[str, bool]:
    return {
        ...
        "terminology_integrity_check": compare_protected_segments(original.technical_terms, revised.technical_terms),
        "numeric_integrity_check": compare_numeric_units_for_integrity(original.numbers, revised.numbers),
...
def normalize_numeric_tokens_for_integrity(text: str) -> list[str]:
    """Extract numeric units with format-only decimal differences normalized."""
    ...
    for match in _NUMBER_RE.finditer(text):
        ...
        tokens.append(f"{core}{suffix}")
    return tokens


def extract_caption_spans(text: str) -> list[str]:
    """Extract only true caption or figure-reference spans for punctuation checks."""

    captions: list[str] = []
    for match in _CAPTION_LINE_RE.finditer(text):
        captions.append(_normalize_protected_text(match.group(0)))
    for match in _CAPTION_FRAGMENT_RE.finditer(text):
        captions.append(_normalize_protected_text(match.group(0)))
    return _deduplicate_preserve_order(captions)


def compare_protected_segments(original: list[str], revised: list[str]) -> bool:
    """Return true when every protected source segment survives after normalization."""

    revised_counter = Counter(_normalize_protected_term(term) for term in revised)
    for term, count in Counter(_normalize_protected_term(term) for term in original).items():
        if revised_counter[term] < count:
            return False
    return True


def compare_numeric_units_for_integrity(original: list[str], revised: list[str]) -> bool:
    """Require every original numeric unit to remain, while ignoring harmless repeated mentions."""

    return set(original).issubset(set(revised))


def narrow_do_not_touch_scope(block_type: str, text: str) -> list[str]:
    """Extract the protected spans that matter for a do-not-touch block."""

    if not text.strip():
        return []
    if block_type == "heading":
        headings = [_normalize_protected_text(item) for item in _HEADING_LINE_RE.findall(text)]
        return headings or [_normalize_protected_text(text)]
    if block_type == "reference":
        references = [_normalize_protected_text(item) for item in _REFERENCE_LINE_RE.findall(text)]
        return references or [_normalize_protected_text(text)]
    if block_type == "english_block":
        placeholders = _extract_placeholders(text)
        if placeholders:
            return placeholders
        return [_normalize_protected_text(text)]
    if block_type == "caption":
        return extract_caption_spans(text) or [_normalize_protected_text(text)]
    if block_type == "placeholder":
        return _extract_placeholders(text)
    if block_type == "formula":
        return [_normalize_protected_text(item) for item in _extract_strict_formula_segments(text)]
    return []


def protected_segments_for_document(block_type: str, text: str) -> list[str]:
    """Extract document-level protected spans by block type for tolerant membership checks."""

    if block_type == "heading":
        return [_normalize_protected_text(item) for item in _HEADING_LINE_RE.findall(text)]
    if block_type == "reference":
        return [_normalize_protected_text(item) for item in _REFERENCE_LINE_RE.findall(text)]
    if block_type == "english_block":
        english_blocks = [_normalize_protected_text(item) for item in _extract_english_blocks(text)]
        return _deduplicate_preserve_order([*english_blocks, *_extract_placeholders(text)])
    if block_type == "caption":
        return extract_caption_spans(text)
    if block_type == "placeholder":
        return _extract_placeholders(text)
    if block_type == "formula":
        return [_normalize_protected_text(item) for item in _extract_strict_formula_segments(text)]
...
def _extract_strict_formula_segments(text: str) -> list[str]:
    formulas: list[str] = []
    formulas.extend(_FORMULA_BLOCK_RE.findall(text))
    formulas.extend(_FORMULA_INLINE_RE.findall(text))
    for line in text.splitlines():
        stripped = line.strip()
        if _FORMULA_LINE_RE.match(stripped) and not _CJK_RE.search(stripped):
...
def _extract_technical_terms(text: str) -> list[str]:
    found: list[str] = []
    for pattern in _TECH_TERM_PATTERNS:
        found.extend(_normalize_protected_term(match.group(0)) for match in pattern.finditer(text))
    return _deduplicate_preserve_order([term for term in found if term])
...
def _normalize_protected_term(text: str) -> str:
